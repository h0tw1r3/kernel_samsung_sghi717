...
static struct clkctl_l2_speed l2_freq_tbl_v2[] = {
	...
	[21] = {1620000,  1, 0x1E, 1225000, 1300000, 5},
	[22] = {1728000,  1, 0x20, 1250000, 1325000, 5},
	[23] = {1836000,  1, 0x22, 1250000, 1350000, 5},
	[24] = {1890000,  1, 0x23, 1250000, 1375000, 5},
};
...
static void __init scpll_init(int pll)
{
	uint32_t regval;
	...
	writel_relaxed(0x0, sc_pll_base[pll] + SCPLL_LUT_A_HW_MAX);
	 ...
	 */
	while (!readl_relaxed(sc_pll_base[pll] + SCPLL_LUT_A_HW_MAX) == 0)
		cpu_relax();
